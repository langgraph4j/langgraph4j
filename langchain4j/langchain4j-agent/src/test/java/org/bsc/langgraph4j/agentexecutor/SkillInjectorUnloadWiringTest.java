...
class SkillInjectorUnloadWiringTest {
    ...
    private static List<List<String>> runWithSnapshots(SkillInjector injector) throws Exception {
        ...
        graph.invoke(GraphInput.args(Map.of("messages", UserMessage.from("hello"))), RunnableConfig.empty()).orElseThrow();
        return snapshots;
    }
    @Test
    ...
    void ephemeralMode_EachCMReturnClearsActiveSkills() throws Exception {
        ...
        graph.invoke(GraphInput.args(Map.of("messages", UserMessage.from("go"))), RunnableConfig.empty()).orElseThrow();
    ...
    void selectiveIds_DropsOnlyListed() throws Exception {
        ...
        graph.invoke(GraphInput.args(Map.of("messages", UserMessage.from("hi"))), RunnableConfig.empty()).orElseThrow();
    ...
    void stickyDefault_NoUnload_ActivePersists() throws Exception {
        ...
        graph.invoke(GraphInput.args(Map.of("messages", UserMessage.from("m"))), RunnableConfig.empty()).orElseThrow();
        assertTrue(before.size() >= 2);
